...
const char* AP_NAME = "HERWATCH";//Web配网模式下的AP-wifi名字
...
extern bool setWiFi_Flag; // 标识是否需要配网，当第一次开机和ui进入配网页面的时候需要重新配网
...
void handleRootPost() {//Post回调函数
  ...
  server.send(200, "text/html", "<meta charset='UTF-8'><h1>保存成功，ESP32手表重启中...</h1>");//返回保存成功页面
  delay(2000);
  //连接wifi
  //connectNewWifi();
